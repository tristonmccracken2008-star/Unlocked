...
import { normalizeResumeLabStore, resumeExperienceKinds, type ResumeDocumentRecord, type ResumeExperienceRecord, type ResumeFactKind, type ResumeFactSource, type ResumeSection, type ResumeTemplate } from "@/data/resume-lab";
...
export type ResumeLabMutation =
  ...
  | { action: "save_resume"; expectedVersion: number; resumeId: string; expectedRecordVersion: number; title: string; contact: ResumeDocumentRecord["contact"]; summary?: string; skills: string[]; sections: ResumeSection[]; template: ResumeTemplate; materialStatus: "draft" | "ready" | "needs_update" }
  | { action: "archive_resume"; expectedVersion: number; resumeId: string; expectedRecordVersion: number }
  | { action: "save_experience"; expectedVersion: number; idempotencyKey?: string; experienceId?: string; expectedRecordVersion?: number; kind: ResumeExperienceRecord["kind"]; organization: string; title: string; location?: string; startDate?: string; endDate?: string; current: boolean; skills: string[]; facts: Array<{ id?: string; kind: ResumeFactKind; text: string; confirmed: boolean; source?: ResumeFactSource; sourceLabel?: string }>; bullets: Array<{ id?: string; text?: string; factIds: string[]; confirmedClaims?: string[]; reviewState?: "draft" | "reviewed" }> }
...
export async function updateResumeLab(user: Pick<AuthUser, "id" | "email" | "name">, mutation: ResumeLabMutation) {
  const result = await mutateResumeLab(user.id, { expectedVersion: mutation.expectedVersion, mutate(current, materials, account) {
    ...
    if (mutation.action === "create_resume" || mutation.action === "duplicate_resume") {
      ...
      const record: ResumeDocumentRecord = { id, materialId, title: mutation.title, kind: mutation.action === "create_resume" ? mutation.kind : "targeted", baseResumeId: mutation.action === "duplicate_resume" ? source!.id : mutation.baseResumeId, target, contact: source?.contact ?? { email: user.email }, summary: source?.summary, sections: source?.sections ?? defaultSections(), skills: source?.skills ?? [], template: source?.template ?? "classic", revisions: [], createdAt: now, updatedAt: now, version: 0 };
      ...
      else { const revision = { id: stableId("resume-revision", user.id, `${currentResume.id}:${currentResume.version}:${currentResume.updatedAt}`), createdAt: now, version: currentResume.version, title: currentResume.title, target: currentResume.target, contact: currentResume.contact, summary: currentResume.summary, sections: currentResume.sections, skills: currentResume.skills, template: currentResume.template }; const record = { ...currentResume, title: mutation.title, contact: mutation.contact, summary: mutation.summary, skills: mutation.skills, sections: mutation.sections, template: mutation.template, revisions: [...(currentResume.revisions ?? []), revision].slice(-30), updatedAt: now, version: currentResume.version + 1 }; resumes[record.id] = record; nextMaterialStore = nextMaterials(materials, record, now, mutation.materialStatus); }
