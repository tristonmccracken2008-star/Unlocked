...
import {
  buildJourneyCommandCenterModel,
  type JourneyCommandRecord,
...
function trackedIds(account: AccountData) {
  return [
    ...new Set([
      ...
      ...(account.savedOpportunities ?? []).map(
...
function recordHref(record: JourneyCommandRecord) {
  return record.applicationWorkspace
...
function deadlineIntent(query: string) {
  return /\b(calendar|deadline|deadlines|due|upcoming|this week|schedule|busy week|busy weeks|conflict|conflicts)\b/i.test(
    query,
  );
}

function conflictIntent(query: string) {
  return /\b(deadline conflict|deadline conflicts|busy week|busy weeks|conflict planning|my schedule)\b/i.test(
    query,
  );
}

function applicationIntent(query: string) {
  return /\b(application|applications|task|tasks|resume|résumé|essay|transcript|recommendation)\b/i.test(
    query,
  );
}

function strategyIntent(query: string) {
  return /^(strategy|my strategy|current mix|my current opportunities|what am i pursuing|opportunity mix)$/i.test(
    query.trim(),
  );
}

function supportedConceptQuery(query: string) {
  return /^(scholarships?|grants?|funding|internships?|research|ai tools?|software|engineering|finance|quant|freshman opportunities?|first year opportunities?|remote opportunities?|student benefits?)$/i.test(
    query.trim(),
  );
}
...
export function buildUniversalSearch(input: {
  ...
  const trackedOpportunities = input.opportunities.filter((item) =>
    ids.has(item.id),
  );
  ...
  const records = [
    ...model.activeRecords,
    ...model.historyGroups.flatMap((group) => group.records),
  ];
  const personal = journeyResults(records, query);
  const savedColleges = input.account.savedColleges ?? [];
  const collegeResults: UniversalSearchResult[] =
    input.account.educationalStage === "high_school"
      ? searchColleges({ query, limit: 4 }).colleges.map((college, index) => ({
          id: `college:${college.id}`,
          kind: "college",
          group: "Colleges",
          title: college.name,
          subtitle: `${college.city}, ${college.state} · ${college.ownership}`,
          href: `/colleges/${college.slug}`,
          score: 1_000 - index,
        }))
      : [];
  const savedCollegeLookup =
    input.account.educationalStage === "high_school"
      ? new Map(
          getColleges(savedColleges.map((record) => record.collegeId)).map(
            (college) => [college.id, college],
          ),
        )
      : new Map();
  const collegeListShortcut: UniversalSearchResult[] =
    input.account.educationalStage === "high_school" &&
    matchScore(query, [
      "my college list",
      "saved colleges",
      ...
      ? [
          {
            id: "college-list:self",
            kind: "college_application",
            group: "My College List",
            title: "My College List",
            subtitle: `${savedColleges.length} saved ${savedColleges.length === 1 ? "college" : "colleges"} · Private`,
            href: "/colleges/saved",
            score: 1_240,
          },
        ]
      : [];
  const savedCollegeResults: UniversalSearchResult[] =
    input.account.educationalStage === "high_school"
      ? savedColleges
          .flatMap((record): UniversalSearchResult[] => {
            ...
            const score = matchScore(query, [
              ...
              record.application
                ? collegeApplicationPlanLabels[record.application.plan]
                : "",
              record.application?.decision
                ? collegeDecisionLabels[record.application.decision.outcome]
                : "",
              ...record.priorities,
            ]);
            if (!score) return [];
          ...
          .slice(0, 4)
      : [];
  const passport: UniversalSearchResult[] = matchScore(query, [
    "my passport",
    ...
    ? [
        {
          ...
          kind: "passport",
          group: "Passport",
          title: "My Opportunity Passport",
          subtitle: "Your private, shareable college journey",
          href: "/passport",
          score: 1_150,
        },
      ]
  ...
  const materials = Object.values(
    normalizeApplicationMaterialStore(input.account.applicationMaterials)
      .records,
  )
    .flatMap((record): UniversalSearchResult[] => {
      const score = matchScore(query, [
        record.title,
        record.versionLabel ?? "",
        applicationMaterialTypeLabels[record.type],
        ...record.contexts,
      ]);
      if (!score) return [];
      return [
        {
          ...
          group: "Materials",
          title: record.title,
          subtitle: `${applicationMaterialTypeLabels[record.type]} · ${applicationMaterialStatusLabels[record.status]}`,
          href: `/materials#material-${encodeURIComponent(record.id)}`,
          score: score + 360,
        },
      ];
    })
  ...
  const resumes = Object.values(resumeLab.resumes)
      ...
      return [
        {
          ...
          group: "Resume Lab",
          title: record.title,
          subtitle: `${record.kind === "master" ? "Master" : "Targeted"} resume · Private`,
          href: `/resume-lab?resume=${encodeURIComponent(record.id)}`,
          score: score + 410,
        },
      ];
    })
    ...
    .slice(0, 4);
  const experiences = Object.values(resumeLab.experiences)
    .flatMap((record): UniversalSearchResult[] => {
      ...
      if (!score) return [];
      const highSchool = input.account.educationalStage === "high_school";
      return [
        {
          id: `experience:${record.id}`,
          kind: highSchool ? "experience" : "resume",
          group: highSchool ? "Activities & Experiences" : "Resume Lab",
          title: record.title ?? "Untitled experience",
          subtitle: `${record.organization ?? "Personal experience"} · Experience Bank`,
          href: highSchool
            ? `/build/experiences/${encodeURIComponent(record.id)}`
            : `/resume-lab?view=experience&experience=${encodeURIComponent(record.id)}`,
          score: score + 390,
        },
      ];
    })
    ...
    .slice(0, 4);
  const answerStories = Object.values(
    normalizeAnswerBank(input.account.answerBank).records,
  )
    .flatMap((record): UniversalSearchResult[] => {
      ...
      return [
        {
          id: `answer-story:${record.id}`,
          kind: "resume",
          group: "Resume Lab",
          title: record.title,
          subtitle: `${record.category} · Private Answer Bank`,
          href: `/answer-bank#story-${encodeURIComponent(record.id)}`,
          score: score + 395,
        },
      ];
    })
    ...
    .slice(0, 4);
  const paths = opportunityPaths
    .flatMap((path): UniversalSearchResult[] => {
      const score = matchScore(query, [
        path.name,
        path.shortName,
        `${path.name} path`,
        ...
        ? [
            {
              ...
              group: "Paths",
              title: path.name,
              subtitle: "Explore opportunities by goal",
              href: `/paths/${path.id}`,
              score: score + 340,
            },
          ]
  ...
  const careerResults = careers
    .flatMap((career): UniversalSearchResult[] => {
      const score = matchScore(query, [
        career.name,
        ...career.aliases,
        career.category,
        ...
        ? [
            {
              ...
              group: "Careers",
              title: career.name,
              subtitle: `${career.category} · ${career.description}`,
    ...
    .slice(0, 4);
  const launchedCollectionIds = new Set(
    opportunityCollectionCoverage(input.opportunities, input.now)
      .filter((item) => item.readiness === "launched")
      .map((item) => item.id),
  );
  const collections = opportunityCollections
    .flatMap((collection): UniversalSearchResult[] => {
      if (!launchedCollectionIds.has(collection.id)) return [];
      const score = matchScore(query, [
        collection.title,
        collection.shortTitle,
        collection.description,
        ...collection.profileAliases,
        `${collection.shortTitle} opportunities`,
      ]);
      return score
        ? [
            {
              ...
              subtitle: "Open a curated starting point",
  ...
  const explorationQuery =
    query
      .replace(
        /^(what (?:can|could) i do with|explore|opportunities (?:in|for)|interested in)\s+/i,
        "",
      )
      .trim() || query;
  const explorer = [
    ...explorerAreas.flatMap((area): UniversalSearchResult[] => {
      const score = matchScore(explorationQuery, [
        area.name,
        area.shortName,
        area.description,
        ...area.aliases,
        ...area.landscapes.flatMap((landscape) => [
          landscape.name,
        ...
        ? [
            {
              ...
              group: "Explore",
              title: area.name,
              subtitle: "Explore related fields and experience types",
    ...
    ...explorerExperienceTypes.flatMap(
      (experience): UniversalSearchResult[] => {
        const score = matchScore(explorationQuery, [
          experience.name,
          experience.description,
          `${experience.name} opportunities`,
        ]);
        return score
          ? [
              {
                id: `explorer-type:${experience.id}`,
                kind: "explorer",
                group: "Explore",
                title: `Explore ${experience.name}`,
                subtitle: experience.description,
                href: `/explore?type=${experience.id}#experience-types`,
                score: score + 350,
              },
            ]
          : [];
  ...
  const accomplishments = buildAccomplishmentsModel({
    ...
    .records.flatMap((record): UniversalSearchResult[] => {
      const score = matchScore(query, [
        record.snapshot.title,
        record.snapshot.organization,
        record.kindLabel,
        record.outcomeLabel,
      ]);
      if (!score) return [];
  ...
  const upcoming = model.calendar.items
    .flatMap((item): UniversalSearchResult[] => {
      if (item.urgency === "overdue" || item.statusAwarePassed) return [];
      const score = deadlineIntent(query)
        ? 700
        : matchScore(query, [
            item.title,
      ...
      return [
        {
          ...
          href: conflictIntent(query)
            ? "/?calendar=conflicts#journey-upcoming-heading"
            : "/#journey-upcoming-heading",
          score:
            score +
            (item.urgency === "today" || item.urgency === "tomorrow" ? 90 : 0),
        },
      ];
    })
    .sort(
  ...
  const tasks = records
    .flatMap(
      (record) =>
        record.applicationWorkspace?.tasks.flatMap(
          (task): UniversalSearchResult[] => {
            if (task.completed) return [];
            const directScore = matchScore(query, [
              task.title,
              record.title,
              record.organization,
            ]);
  ...
  const preciseCatalog = catalog.opportunities.filter(
    (item) =>
      supportedConceptQuery(query) ||
      matchScore(query, [
        item.title,
        item.organization,
  ...
  const opportunities = preciseCatalog.map(
    (item, index): UniversalSearchResult => ({
