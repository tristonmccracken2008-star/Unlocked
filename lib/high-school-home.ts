...
export function buildHighSchoolHomeSummary(input:{data:AccountData;firstName:string;opportunities:Opportunity[];now?:Date}):HighSchoolHomeSummary {
  ...
  const comingUp=[...verifiedCollegeDates,...verifiedOpportunityDates,...testDates,...journeyDates,...bluebookDates,...writingDates].sort((a,b)=>a.date.localeCompare(b.date)||Number(a.provenance==="Your date")-Number(b.provenance==="Your date")).slice(0,4);
