...
export type FeedbackType = "helpful" | "not-relevant" | "already-completed" | "already-applied" | "too-expensive" | "too-time-consuming" | "completed" | "dismissed" | "dont-enjoy-this" | "prefer-research" | "prefer-industry" | "not-interested" | "show-fewer" | "not-eligible" | "undo";
...
export type ForYouComparisonProjection = {
  ...
  matchReason: string;
  journeyContribution?: string;
};

export type ForYouWatchingItem = {
  opportunityId: string;
  title: string;
  organization: string;
  href: string;
...
export type ForYouRecommendationInsight = {
  ...
  priorityLabel: "Best fit" | "Deadline soon" | "Adds something new" | "Strong fit";
  factLine: string;
  whyThisOne: string;
  facts: ForYouDecisionFact[];
  comparison: ForYouComparisonProjection;
  state: {
    watched: boolean;
    inJourney: boolean;
