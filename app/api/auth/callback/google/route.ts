...
import { accountHasCompletedOnboarding, createSession, oauthStateCookieName, readAccountData, sessionCookieName, upsertUser } from "@/lib/auth-store";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.redirect(`${appUrl()}/${accountHasCompletedOnboarding(accountData) ? "" : "?auth=signed-in"}`);
