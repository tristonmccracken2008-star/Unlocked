...
import { resumeExperienceKinds, resumeFactKinds, resumeSectionKinds, type ResumeSection } from "@/data/resume-lab";
...
function parseMutation(value: unknown): ResumeLabMutation {
  ...
  if (action === "save_experience") {
    ...
    const facts = Array.isArray(body.facts) ? body.facts.slice(0, 80).flatMap((raw): Array<{ id?: string; kind: typeof resumeFactKinds[number]; text: string; confirmed: boolean }> => { if (!raw || typeof raw !== "object" || Array.isArray(raw)) return []; const fact = raw as Record<string, unknown>; const text = clean(fact.text, 300); return text && resumeFactKinds.includes(fact.kind as never) ? [{ id: id(fact.id) || undefined, kind: fact.kind as typeof resumeFactKinds[number], text, confirmed: Boolean(fact.confirmed) }] : []; }) : [];
    const bullets = Array.isArray(body.bullets) ? body.bullets.slice(0, 100).flatMap((raw): Array<{ id?: string; text?: string; factIds: string[]; confirmedClaims?: string[] }> => { if (!raw || typeof raw !== "object" || Array.isArray(raw)) return []; const bullet = raw as Record<string, unknown>; return [{ id: id(bullet.id) || undefined, text: clean(bullet.text, 500) || undefined, factIds: strings(bullet.factIds, 12, 160).filter((item) => safeId.test(item)), confirmedClaims: strings(bullet.confirmedClaims, 20, 80) }]; }) : [];
