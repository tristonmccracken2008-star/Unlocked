...
export default async function ResumeLabPage({ searchParams }: { searchParams: Promise<{ target?: string; returnTo?: string }> }) {
  const session = await requireCompletedOnboarding();
  const { target, returnTo } = await searchParams;
  ...
  const safeReturnTo = returnTo?.startsWith("/applications/") && !returnTo.startsWith("//") ? returnTo : undefined;
  return <ResumeLab initial={buildResumeLabModel({ user: session.user, account: session.data, opportunities })} initialTargetId={target && ids.includes(target) ? target : undefined} returnTo={safeReturnTo} />;
}
