...
export default async function ResumeLabPage({ searchParams }: { searchParams: Promise<{ target?: string }> }) {
  ...
  return <ResumeLab initial={buildResumeLabModel({ user: session.user, account: session.data, opportunities })} initialTargetId={target && ids.includes(target) ? target : undefined} />;
}
