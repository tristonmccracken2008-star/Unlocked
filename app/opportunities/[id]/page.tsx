...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    <aside className="h-fit rounded-[2rem] bg-white p-5 shadow-soft ring-1 ring-ink/8 lg:sticky lg:top-6"><p className="rule-label text-forest">Trust & verification</p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={displayedStatus}/><ConfidenceBadge status={displayedStatus}/></div><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-ink/40">Official provider</dt><dd className="mt-1 font-bold">{item.organization}</dd></div><div><dt className="text-ink/40">Last verified</dt><dd className="mt-1 font-bold">{formatDate(item.last_verified)}</dd></div><div><dt className="text-ink/40">Source</dt><dd className="mt-1 break-words text-xs text-ink/55">{item.official_source}</dd></div></dl><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-6 flex min-h-11 items-center justify-center gap-2 rounded-full bg-ink px-4 text-sm font-bold text-white hover:bg-forest">Official source <ArrowIcon/></a><ReportOutdatedButton opportunityId={item.id}/><section className="mt-6 border-t border-ink/10 pt-5"><h2 className="font-editorial text-xl font-bold">Why trust this?</h2><p className="mt-3 text-xs leading-5 text-ink/50">UnlockED verifies listings against official provider or university sources and regularly reviews published details. Missing information is shown as unknown rather than inferred.</p><p className="mt-3 text-[11px] leading-5 text-ink/40">UnlockED does not administer this opportunity or determine eligibility.</p></section></aside>
    </div></main>
  </>
}
