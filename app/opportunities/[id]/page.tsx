...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    <header className="border-b border-ink/15 bg-white px-5 py-12 sm:px-8 sm:py-16"><div className="mx-auto max-w-6xl">
      ...
      <div className="mt-7 grid gap-8 border-t border-ink/20 pt-7 lg:grid-cols-[minmax(0,1fr)_300px]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.type}</span><span className="rule-label text-ink/40">{item.category}</span><StatusBadge status={displayedStatus}/></div><p className="mt-5 text-sm font-bold uppercase tracking-widest text-ink/40">{item.organization}</p><h1 className="mt-2 font-editorial text-4xl font-bold leading-tight sm:text-5xl">{item.title}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-ink/60">{item.description}</p></div><aside className="border-t-2 border-ink pt-5 lg:border-l-2 lg:border-t-0 lg:pl-6 lg:pt-0"><p className="rule-label text-ink/40">Last verified</p><p className="mt-2 font-bold">{item.last_verified}</p><OpportunityActivityActions opportunityId={item.id} type={item.type} officialSource={item.official_source}/></aside></div>
    ...
    <aside className="h-fit border-y border-ink/20 bg-white p-5 lg:sticky lg:top-6"><p className="rule-label text-forest">Trust & verification</p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={displayedStatus}/><ConfidenceBadge status={displayedStatus}/></div><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-ink/40">Official provider</dt><dd className="mt-1 font-bold">{item.organization}</dd></div><div><dt className="text-ink/40">Last verified</dt><dd className="mt-1 font-bold">{item.last_verified}</dd></div><div><dt className="text-ink/40">Source</dt><dd className="mt-1 break-words text-xs text-ink/55">{item.official_source}</dd></div></dl><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-6 flex min-h-11 items-center justify-center gap-2 bg-ink px-4 text-xs font-bold uppercase tracking-wider text-white">Official source <ArrowIcon/></a><ReportOutdatedButton opportunityId={item.id}/><section className="mt-6 border-t border-ink/20 pt-5"><h2 className="font-editorial text-xl font-bold">Why trust this?</h2><p className="mt-3 text-xs leading-5 text-ink/50">UnlockED verifies listings against official provider or university sources and regularly reviews published details. Confidence labels explain the current review state; missing information is shown as unknown rather than inferred.</p><p className="mt-3 text-[11px] leading-5 text-ink/40">UnlockED does not administer this opportunity or determine eligibility.</p></section></aside>
    </div></main>
  </>
}
