...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    <header className="border-b border-ink/15 bg-white px-5 py-12 sm:px-8 sm:py-16"><div className="mx-auto max-w-6xl">
      <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wider text-ink/40"><Link href="/">Dashboard</Link><span>/</span><Link href={itemGoal[1]} className="text-forest">{itemGoal[0]}</Link><span>/</span><Link href="/opportunities">Opportunity guide</Link></nav>
      <div className="mt-7 grid gap-8 border-t border-ink/20 pt-7 lg:grid-cols-[minmax(0,1fr)_300px]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.type}</span><span className="rule-label text-ink/40">{item.category}</span><StatusBadge status={item.verification_status}/></div><p className="mt-5 text-sm font-bold uppercase tracking-widest text-ink/40">{item.organization}</p><h1 className="mt-2 font-editorial text-4xl font-bold leading-tight sm:text-5xl">{item.title}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-ink/60">{item.description}</p></div><aside className="border-t-2 border-ink pt-5 lg:border-l-2 lg:border-t-0 lg:pl-6 lg:pt-0"><p className="rule-label text-ink/40">Last verified</p><p className="mt-2 font-bold">{item.last_verified}</p><OpportunityActivityActions opportunityId={item.id} type={item.type} officialSource={item.official_source}/></aside></div>
      <section aria-labelledby="quick-facts" className="mt-8"><h2 id="quick-facts" className="rule-label text-forest">Quick Facts</h2><dl className="mt-3 grid border-l border-t border-ink/15 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Estimated value" value={valueLabel(item)}/><Detail label="Time required" value={timeRequired(item)}/><Detail label="Difficulty" value={item.difficulty??"Not published by the official source reviewed."}/><Detail label="Best for" value={bestFor(item)}/></dl></section>
    </div></header>
    ...
    <main className="px-5 py-10 sm:px-8 sm:py-14"><div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,1fr)_260px]"><article className="space-y-12">
      ...
      <section aria-labelledby="why-this-matters" className="border-l-2 border-gold bg-white px-5 py-6 sm:px-7"><p className="rule-label text-forest">Student impact</p><h2 id="why-this-matters" className="mt-2 font-editorial text-3xl font-bold">Why This Matters</h2><div className="mt-4 space-y-2">{whyThisMatters(item).map((sentence)=><p key={sentence} className="text-sm leading-6 text-ink/65">{sentence}</p>)}</div></section>
      ...
      <GuideSection eyebrow="Similar opportunities" title="Continue exploring verified options"><div className="grid border-l border-t border-ink/15 sm:grid-cols-2">{related.map((candidate)=><Link key={candidate.id} href={`/opportunities/${candidate.id}`} className="group border-b border-r border-ink/15 bg-white p-5"><div className="flex flex-wrap items-center gap-2"><span className="rule-label text-forest">{candidate.type}</span><StatusBadge status={candidate.verification_status}/><ConfidenceBadge status={candidate.verification_status}/></div><h3 className="mt-3 font-editorial text-xl font-bold group-hover:text-forest">{candidate.title}</h3><p className="mt-2 text-xs font-bold uppercase tracking-wider text-ink/35">{candidate.organization}</p><p className="mt-3 line-clamp-2 text-sm leading-6 text-ink/50">{candidate.description}</p><span className="mt-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider">Read guide <ArrowIcon/></span></Link>)}</div></GuideSection>
    </article>

    <aside className="h-fit border-y border-ink/20 bg-white p-5 lg:sticky lg:top-6"><p className="rule-label text-forest">Trust & verification</p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={item.verification_status}/><ConfidenceBadge status={item.verification_status}/></div><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-ink/40">Official provider</dt><dd className="mt-1 font-bold">{item.organization}</dd></div><div><dt className="text-ink/40">Last verified</dt><dd className="mt-1 font-bold">{item.last_verified}</dd></div><div><dt className="text-ink/40">Source</dt><dd className="mt-1 break-words text-xs text-ink/55">{item.official_source}</dd></div></dl><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-6 flex min-h-11 items-center justify-center gap-2 bg-ink px-4 text-xs font-bold uppercase tracking-wider text-white">Official source <ArrowIcon/></a><Link href={`/contact?opportunity=${encodeURIComponent(item.title)}`} className="mt-3 flex min-h-11 items-center justify-center border border-ink/20 px-4 text-center text-xs font-bold uppercase tracking-wider text-ink/60">Report outdated information</Link><section className="mt-6 border-t border-ink/20 pt-5"><h2 className="font-editorial text-xl font-bold">Why trust this?</h2><p className="mt-3 text-xs leading-5 text-ink/50">UnlockED verifies listings against official provider or university sources and regularly reviews published details. Confidence labels explain the current review state; missing information is shown as unknown rather than inferred.</p><p className="mt-3 text-[11px] leading-5 text-ink/40">UnlockED does not administer this opportunity or determine eligibility.</p></section></aside>
    </div></main>
  </>
}
