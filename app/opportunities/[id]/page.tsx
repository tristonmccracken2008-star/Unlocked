...
export default async function Page({params}:{params:Promise<{id:string}>}){
  ...
  const lifecyclePresentation:OpportunityLifecyclePresentation={state:lifecycle.state,displayState:lifecycle.displayState,confidence:lifecycle.confidence,label:lifecycle.label,actionable:lifecycle.actionable,recommendationEligible:lifecycle.recommendationEligible,recurring:lifecycle.recurring,actionLabel:lifecycle.actionLabel,actionAllowed:lifecycle.actionAllowed};
  const itemGoal=goalPath(item);
  const related=getRelatedOpportunities(item,12,catalog).filter((candidate)=>resolveOpportunityLifecycle(candidate).recommendationEligible).slice(0,4);
  ...
  const faq=[
    {q:`Is ${item.title} currently available?`,a:`UnlockED currently shows “${lifecycle.label}” with ${lifecycle.confidence} lifecycle confidence. The record was last reviewed on ${item.last_verified}. Always confirm current terms on the official source.`},
  ...
  return <>
    ...
    <header className="bg-white px-5 py-12 sm:px-8 sm:py-20"><div className="mx-auto max-w-6xl">
      <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-xs font-bold text-ink/40"><Link href="/opportunities" className="text-forest">Discover</Link><span>/</span><span>{item.type}</span></nav>
      <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.type}</span><span className="rule-label text-ink/40">{item.category}</span><LifecycleBadge state={lifecycle.displayState} confidence={lifecycle.confidence} label={lifecycle.label}/></div><div className="mt-6 flex items-center gap-4"><OrganizationLogo opportunity={item} size="lg"/><p className="text-sm font-bold uppercase tracking-widest text-ink/35">{item.organization}</p></div><h1 className="mt-4 max-w-4xl font-editorial text-5xl font-bold leading-[1] tracking-[-.05em] sm:text-7xl">{item.title}</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-ink/60">{item.description}</p></div><aside className="rounded-[2rem] bg-paper p-5 shadow-soft"><p className="rule-label text-forest">Official next step</p><OpportunityActivityActions opportunityId={item.id} type={item.type} officialSource={item.official_source} lifecycle={lifecyclePresentation}/><p className="mt-4 text-xs leading-5 text-ink/50">{lifecycle.actionable?"UnlockED’s current evidence supports an active official application path.":"UnlockED has not confirmed an active application window. You can still keep this opportunity in Journey."}</p><div className="mt-5 border-t border-ink/10 pt-5"><p className="rule-label text-ink/35">Last verified</p><p className="mt-2 font-bold">{formatDate(item.last_verified)}</p></div></aside></div>
    ...
    <main className="px-5 py-12 sm:px-8 sm:py-16"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[minmax(0,1fr)_280px]"><article className="space-y-14">
      ...
      <GuideSection eyebrow="Timeline" title="Dates and current availability"><dl className="grid gap-3 sm:grid-cols-2"><Detail label="Current status" value={`${lifecycle.label}. ${lifecycle.confidence==="confirmed"?"Confirmed by current evidence.":lifecycle.confidence==="strong"?"Supported by current structured evidence.":"Check the official source before acting."}`}/><Detail label="Opens" value={lifecycle.openingDate?.normalizedValue?formatLifecycleDate(lifecycle.openingDate.normalizedValue,lifecycle.openingDate.estimated):unknown("the opening date")}/><Detail label="Priority deadline" value={lifecycle.priorityDeadline?.normalizedValue?formatLifecycleDate(lifecycle.priorityDeadline.normalizedValue,lifecycle.priorityDeadline.estimated):unknown("a priority deadline")}/><Detail label="Final deadline" value={lifecycle.finalDeadline?.normalizedValue?formatLifecycleDate(lifecycle.finalDeadline.normalizedValue,lifecycle.finalDeadline.estimated):deadlineLabel(item)}/><Detail label="Decision timeline" value={lifecycle.decisionDate?.normalizedValue?formatLifecycleDate(lifecycle.decisionDate.normalizedValue,lifecycle.decisionDate.estimated):unknown("the decision or approval timeline")}/><Detail label="Renewal / recurrence" value={lifecycle.recurrence?`${lifecycle.recurrence.type.replaceAll("_"," ")} recurrence; ${lifecycle.recurrence.confidence} confidence. This does not mean the current cycle is open.`:renewal(item)}/></dl></GuideSection>
      ...
      {related.length ? <GuideSection eyebrow="Similar opportunities" title="Explore related listings"><div className="grid gap-3 sm:grid-cols-2">{related.map((candidate)=><Link key={candidate.id} href={`/opportunities/${candidate.id}`} className="group rounded-[1.5rem] bg-white p-5 ring-1 ring-ink/8"><div className="flex flex-wrap items-center gap-2"><span className="rule-label text-forest">{candidate.type}</span><StatusBadge status={candidate.verification_status}/></div><div className="mt-4 flex items-start gap-3"><OrganizationLogo opportunity={candidate} size="sm"/><div className="min-w-0"><h3 className="font-editorial text-xl font-bold group-hover:text-forest">{candidate.title}</h3><p className="mt-2 text-xs font-bold uppercase tracking-wider text-ink/35">{candidate.organization}</p></div></div><p className="mt-3 line-clamp-2 text-sm leading-6 text-ink/50">{candidate.description}</p><span className="mt-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider">Read guide <ArrowIcon/></span></Link>)}</div></GuideSection> : null}
    </article>

    <aside className="h-fit rounded-[2rem] bg-white p-5 shadow-soft ring-1 ring-ink/8 lg:sticky lg:top-6"><p className="rule-label text-forest">Trust & verification</p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={displayedStatus}/><ConfidenceBadge status={displayedStatus}/></div><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-ink/40">Official provider</dt><dd className="mt-1 font-bold">{item.organization}</dd></div><div><dt className="text-ink/40">Last verified</dt><dd className="mt-1 font-bold">{formatDate(item.last_verified)}</dd></div><div><dt className="text-ink/40">Source</dt><dd className="mt-1 break-words text-xs text-ink/55">{item.official_source}</dd></div></dl><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-6 flex min-h-11 items-center justify-center gap-2 rounded-full bg-ink px-4 text-sm font-bold text-white hover:bg-forest">Official source <ArrowIcon/><span className="sr-only">(opens in a new tab)</span></a><ReportOutdatedButton opportunityId={item.id}/><section className="mt-6 border-t border-ink/10 pt-5"><h2 className="font-editorial text-xl font-bold">Why trust this?</h2><p className="mt-3 text-xs leading-5 text-ink/50">UnlockED verifies listings against official provider or university sources and regularly reviews published details. Missing information is shown as unknown rather than inferred.</p><p className="mt-3 text-[11px] leading-5 text-ink/40">UnlockED does not administer this opportunity or determine eligibility.</p></section></aside>
    </div></main>
  </>
}
...
function formatLifecycleDate(value:string,estimated:boolean){const date=value.slice(0,10);return`${estimated?"Estimated ":""}${formatDate(date)}`}
