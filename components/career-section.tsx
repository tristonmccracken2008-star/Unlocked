...
export function CareerSection({ major, year }: { major: string; year: string }) {
  ...
  const card = (item: Opportunity, index: number) => <article key={item.id} className="grid gap-4 border-b border-ink/20 bg-white px-4 py-5 last:border-b-0 lg:grid-cols-[36px_1fr_190px_150px] lg:items-start">
    ...
    <div className="lg:text-right"><p className="rule-label text-ink/35">Editorial estimate</p><p className="mt-2 text-xs"><span className="font-bold">Difficulty:</span> {item.difficulty}</p><p className="mt-1 text-xs"><span className="font-bold">Prestige:</span> {item.prestige}</p><p className="mt-3 text-[11px] text-ink/35">Verified {item.last_verified}</p><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-11 items-center gap-2 border-b border-ink pb-1 text-xs font-bold uppercase tracking-wider hover:border-forest hover:text-forest">Official source <ArrowIcon /></a></div>
  </article>;
