...
import { academicYears, deadlineLabel, opportunities, opportunityCategories, opportunityMajors, type Compensation, type Opportunity, type OpportunityCategory, type WorkMode } from "@/data/opportunities";
...
function relevance(opportunity: Opportunity, major: string, year: string) {
  ...
  if (opportunity.academicYears.includes(year)) score += 4;
...
export function CareerSection({ major, year }: { major: string; year: string }) {
  ...
  const personalized = useMemo(() => [...opportunities].sort((a,b) => relevance(b,major,year)-relevance(a,major,year) || a.title.localeCompare(b.title)).slice(0,6), [major,year]);
  const visible = useMemo(() => {
    const search = query.trim().toLowerCase();
    return opportunities.filter((item) => {
      const deadlineMatches = deadlineFilter === "All" || (deadlineFilter === "Published" && Boolean(item.applicationDeadline)) || (deadlineFilter === "Rolling or varies" && ["rolling","varies"].includes(item.deadlineType)) || (deadlineFilter === "Not announced" && item.deadlineType === "not_announced");
      return (!search || `${item.title} ${item.organization} ${item.description} ${item.majors.join(" ")}`.toLowerCase().includes(search)) && (majorFilter === "All" || item.majors.includes("Any Major") || item.majors.includes(majorFilter)) && (yearFilter === "All" || item.academicYears.includes(yearFilter)) && deadlineMatches && (paidFilter === "All" || item.compensation === paidFilter) && (modeFilter === "All" || item.workMode === modeFilter) && (categoryFilter === "All" || item.category === categoryFilter);
  ...
  const card = (item: Opportunity, index: number) => <article key={item.slug} className="grid gap-4 border-b border-ink/20 bg-white px-4 py-5 last:border-b-0 lg:grid-cols-[36px_1fr_190px_150px] lg:items-start">
    <span className="hidden pt-1 font-mono text-xs text-ink/30 lg:block">{String(index+1).padStart(2,"0")}</span>
    <div><div className="flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.category}</span><span className="rule-label border-l border-ink/20 pl-3 text-trust">Recently Verified</span><span className="rule-label border-l border-ink/20 pl-3 text-ink/40">{item.scope === "national" ? "National" : "School-specific"}</span></div><h3 className="mt-2 font-editorial text-xl font-bold">{item.title}</h3><p className="mt-1 text-xs font-bold uppercase tracking-wider text-ink/35">{item.organization}</p><p className="mt-3 text-sm leading-6 text-ink/55">{item.description}</p><p className="mt-3 text-xs leading-5 text-ink/45"><span className="font-bold">Eligibility:</span> {item.eligibility}</p><p className="mt-2 text-xs text-ink/45">{item.majors.join(" · ")} · {item.academicYears.join(" · ")}</p></div>
    <div className="border-l-2 border-forest pl-4"><p className="rule-label text-ink/35">Application deadline</p><p className="mt-2 font-editorial text-xl font-bold text-forest">{deadlineLabel(item)}</p><p className="mt-4 text-xs text-ink/45">{item.location}</p><p className="mt-2 text-xs font-bold">{item.compensation} · {item.workMode}</p></div>
    <div className="lg:text-right"><p className="rule-label text-ink/35">Editorial estimate</p><p className="mt-2 text-xs"><span className="font-bold">Difficulty:</span> {item.difficulty}</p><p className="mt-1 text-xs"><span className="font-bold">Prestige:</span> {item.prestige}</p><p className="mt-3 text-[11px] text-ink/35">Verified {item.lastVerifiedAt}</p><a href={item.officialSourceUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 border-b border-ink pb-1 text-xs font-bold uppercase tracking-wider hover:border-forest hover:text-forest">Official source <ArrowIcon /></a></div>
  </article>;
  ...
  return <section className="border-t-2 border-ink px-5 py-10 sm:px-8" aria-labelledby="career-title">
    ...
    <div className="mt-12 flex items-end justify-between gap-4 border-b-2 border-ink pb-4"><div><p className="rule-label text-forest">Career directory</p><h2 className="mt-2 font-editorial text-3xl font-bold">All verified opportunities</h2></div><p className="text-sm font-bold text-ink/45">{visible.length} of {opportunities.length}</p></div>
