...
export function CareerSection({ major, year }: { major: string; year: string }) {
  ...
  const card = (item: Opportunity, index: number) => <article key={item.id} className="grid gap-4 border-b border-ink/20 bg-white px-4 py-5 last:border-b-0 lg:grid-cols-[36px_1fr_190px_150px] lg:items-start">
    ...
    <div><div className="flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.category}</span><span className="rule-label border-l border-ink/20 pl-3 text-trust">Recently Verified</span><span className="rule-label border-l border-ink/20 pl-3 text-ink/40">{item.school_scope}</span></div><h3 className="mt-2 font-editorial text-xl font-bold">{item.title}</h3><p className="mt-1 text-xs font-bold uppercase tracking-wider text-ink/35">{item.organization}</p><p className="mt-3 text-sm leading-6 text-ink/55">{item.description}</p><p className="mt-3 text-xs leading-5 text-ink/45"><span className="font-bold">Eligibility:</span> {item.eligibility}</p><p className="mt-2 text-xs text-ink/45">{item.majors.join(" · ")} · {item.academic_years.join(" · ")}</p></div>
    <div className="border-l-2 border-forest pl-4"><p className="rule-label text-ink/35">Application deadline</p><p className="mt-2 font-editorial text-xl font-bold text-forest">{deadlineLabel(item)}</p><p className="mt-4 text-xs text-ink/45">{item.location}</p><p className="mt-2 text-xs font-bold">{item.metadata.compensation} · {item.metadata.workMode}</p></div>
    <div className="lg:text-right"><p className="rule-label text-ink/35">Editorial estimate</p><p className="mt-2 text-xs"><span className="font-bold">Difficulty:</span> {item.difficulty}</p><p className="mt-1 text-xs"><span className="font-bold">Prestige:</span> {item.prestige}</p><p className="mt-3 text-[11px] text-ink/35">Verified {item.last_verified}</p><a href={item.official_source} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 border-b border-ink pb-1 text-xs font-bold uppercase tracking-wider hover:border-forest hover:text-forest">Official source <ArrowIcon /></a></div>
  </article>;
