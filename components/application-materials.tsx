...
function MaterialForm({ record, pending, onSubmit, onCancel }: {
  ...
  return <form className={styles.form} onSubmit={(event) => { event.preventDefault(); onSubmit({ type, title: title.trim(), versionLabel: versionLabel.trim() || undefined, status, contexts, notes: notes.trim() || undefined }); }}>
    ...
    <p className={styles.fileNotice}>UnlockED stores this record, not the document itself. Private file uploads are intentionally unavailable until audited storage is configured.</p>
...
export function ApplicationMaterials({ initial, guidance }: { initial: ApplicationMaterialsModel; guidance: GuidanceState }) {
  ...
  return <main className={styles.page} data-application-materials="">
    <div className={styles.container}>
      ...
      {!model.records.length ? <SmartEmptyState className={styles.empty} eyebrow="Materials" title="Keep reusable application materials here." description="Add a record for a resume, transcript, essay, or other material. UnlockED will connect it to verified application requirements without storing the document itself." primaryAction={{ label: "Add your first material", onClick: () => { if (addRef.current) addRef.current.open = true; } }} icon={ListIcon} /> : <>
