...
const totalSteps = 6;
const stepIds = ["school", "graduation-year", "major", "career-goal", "opportunity-interests", "current-priority"] as const;
...
function Progress({ step }: { step: number }) {
  return <div className="w-full max-w-xs" role="progressbar" aria-label="Onboarding progress" aria-valuemin={1} aria-valuemax={totalSteps} aria-valuenow={step + 1} aria-valuetext={`Step ${step + 1} of ${totalSteps}`}>
    <div className="grid grid-cols-6 gap-1.5" aria-hidden="true">{Array.from({ length: totalSteps }, (_, index) => <span key={index} className={`h-1.5 rounded-full transition-all duration-300 motion-reduce:transition-none ${index <= step ? "bg-forest" : "bg-ink/12"}`} />)}</div>
...
function Welcome() {
  return <div className="mx-auto max-w-xl text-center">
    ...
    <h1 tabIndex={-1} data-onboarding-heading className="mt-3 font-editorial text-4xl font-bold leading-tight tracking-[-.035em] text-ink outline-none sm:text-5xl">Welcome to UnlockED</h1>
    <p className="mx-auto mt-4 max-w-md text-sm leading-7 text-ink/58">Six quick questions help UnlockED check eligibility and select opportunities that fit. You can refine your profile later.</p>
  </div>;
}
...
function Question(props: {
  ...
  if (step === 4) return <QuestionShell eyebrow="Interests" title="What kinds of opportunities matter most to you?" helper="Used to balance your shortlist across the opportunity types you care about."><ChoiceGrid options={opportunityInterestOptions} values={draft.interests} onToggle={(value) => update({ interests: draft.interests.includes(value) ? draft.interests.filter((item) => item !== value) : [...draft.interests, value] })} /></QuestionShell>;
...
function QuestionShell({ eyebrow, title, helper, children }: { eyebrow: string; title: string; helper?: string; children: React.ReactNode }) {
  return <div className="mx-auto w-full max-w-xl text-center">
    ...
    <h1 tabIndex={-1} data-onboarding-heading className="mt-3 font-editorial text-4xl font-bold leading-tight tracking-[-.035em] text-ink outline-none sm:text-5xl">{title}</h1>
