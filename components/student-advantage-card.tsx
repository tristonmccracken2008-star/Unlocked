...
export function StudentAdvantageCard({ profile, school }: { profile: StudentProfile; school: School }) {
  ...
  return <section className="border-b border-ink/15 py-8" aria-labelledby="advantage-title"><div className="grid gap-7 lg:grid-cols-[1fr_1fr] lg:items-center"><div><p className="rule-label text-forest">Your Student Advantage</p><div className="mt-2 flex items-baseline justify-between gap-4"><h2 id="advantage-title" className="font-editorial text-2xl font-bold sm:text-3xl">Overall score</h2><p className="font-editorial text-4xl font-bold text-forest">{result.score}<span className="ml-1 text-lg text-ink/30">/100</span></p></div><div className="mt-4 h-1.5 bg-ink/10" role="progressbar" aria-valuenow={result.score} aria-valuemin={0} aria-valuemax={100} aria-label="Student Advantage Score"><div className="h-full bg-gold transition-[width]" style={{width:`${result.score}%`}}/></div><p className="mt-3 text-xs text-ink/45">{result.viewed} opportunities viewed · {result.saved} saved</p></div>
