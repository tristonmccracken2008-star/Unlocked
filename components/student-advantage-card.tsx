...
export function StudentAdvantageCard({ profile, school }: { profile: StudentProfile; school: School }) {
  ...
  return <section className="border-y border-ink/10 py-9" aria-labelledby="advantage-title"><div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-center"><div className="flex items-center gap-6"><div role="progressbar" aria-valuenow={result.score} aria-valuemin={0} aria-valuemax={100} aria-label="Student Advantage Score" className="grid h-28 w-28 shrink-0 place-items-center rounded-full" style={{background:`conic-gradient(#b78318 ${result.score*3.6}deg, rgba(16,36,62,.08) 0)`}}><div className="grid h-24 w-24 place-items-center rounded-full bg-white text-center"><p className="font-editorial text-3xl font-bold text-forest">{result.score}<span className="block text-[10px] font-sans font-bold uppercase tracking-wider text-ink/35">out of 100</span></p></div></div><div><p className="rule-label text-forest">Your Student Advantage</p><h2 id="advantage-title" className="mt-2 font-editorial text-2xl font-bold sm:text-3xl">Your progress at a glance</h2><p className="mt-2 text-sm leading-6 text-ink/45">{result.viewed} opportunities viewed · {result.saved} saved</p></div></div>
    <div className="border-l-2 border-gold pl-5"><p className="rule-label text-ink/40">Potential value remaining</p><p className="mt-1 font-editorial text-3xl font-bold text-forest">{money.format(result.remaining)}+</p><p className="mt-1 text-[11px] leading-5 text-ink/40">Verified opportunities with documented values only.</p>{result.next&&<Link href={`/opportunities/${result.next.id}`} className="group mt-4 flex items-center justify-between gap-4 border-t border-ink/15 pt-3"><div><p className="rule-label text-ink/35">Next best action</p><p className="mt-1 text-sm font-bold group-hover:text-forest">{result.next.type==="Benefit"||result.next.type==="AI"?"Claim":"Explore"} {result.next.title}</p></div><ArrowIcon/></Link>}</div></div>
    <details className="group mt-6 border-t border-ink/10 pt-4"><summary className="flex cursor-pointer list-none items-center justify-between text-xs font-bold uppercase tracking-wider text-ink/55"><span>View score breakdown</span><span className="text-lg font-normal group-open:rotate-45">+</span></summary><div className="mt-4 grid grid-cols-2 border-l border-t border-ink/15 sm:grid-cols-3">{rows.map(([label,value])=><div key={label} className="border-b border-r border-ink/15 p-3"><p className="text-xs font-bold text-ink/55">{label}</p><p className="mt-1 font-editorial text-lg font-bold">{label==="Profile complete"&&value===100?<span className="inline-flex items-center gap-1 text-trust"><CheckIcon className="h-4 w-4"/> Complete</span>:`${value}%`}</p></div>)}</div><p className="mt-3 text-xs text-ink/45">{result.schoolAvailable?`${result.schoolPercent}% of verified school-specific opportunities explored`:"No verified school-specific opportunities available yet"}</p></details>
  </section>;
}
