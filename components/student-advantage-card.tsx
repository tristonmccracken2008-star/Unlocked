...
export function StudentAdvantageCard({ profile, school }: { profile: StudentProfile; school: School }) {
  ...
  return <section className="bg-paper px-5 py-8 shadow-[0_12px_35px_rgba(16,36,62,.06)] sm:px-8 sm:py-9" aria-labelledby="advantage-title"><div className="grid gap-8 md:grid-cols-[200px_minmax(0,1fr)] md:gap-10"><div className="flex flex-col items-center text-center md:items-start md:text-left"><p className="rule-label text-forest">Your Student Advantage</p><div role="progressbar" aria-valuenow={result.score} aria-valuemin={0} aria-valuemax={100} aria-label="Student Advantage Score" className="mt-4 grid h-32 w-32 place-items-center rounded-full" style={{background:`conic-gradient(#b78318 ${result.score*3.6}deg, rgba(16,36,62,.08) 0)`}}><div className="grid h-[7rem] w-[7rem] place-items-center rounded-full bg-paper text-center"><p className="font-editorial text-4xl font-bold text-forest">{result.score}<span className="block text-[10px] font-sans font-bold uppercase tracking-wider text-ink/35">out of 100</span></p></div></div><h2 id="advantage-title" className="mt-5 font-editorial text-xl font-bold">{insight}</h2><p className="mt-2 text-xs leading-5 text-ink/45">{result.viewed} viewed · {result.saved} saved</p></div>
    <div className="flex flex-col justify-center">{result.next&&<Link href={`/opportunities/${result.next.id}`} className="group border-l-2 border-gold bg-white/70 px-5 py-4"><p className="rule-label text-forest">Next best action</p><div className="mt-2 flex items-center justify-between gap-5"><div><p className="font-editorial text-xl font-bold group-hover:text-forest">{result.next.type==="Benefit"||result.next.type==="AI"?"Claim":"Explore"} {result.next.title}</p><p className="mt-1 text-xs text-ink/45">The strongest unviewed match for your profile.</p></div><ArrowIcon className="h-4 w-4 shrink-0"/></div></Link>}<div className="mt-6"><p className="rule-label text-ink/40">Potential value remaining</p><p className="mt-2 font-editorial text-3xl font-bold text-forest">{money.format(result.remaining)}+</p><p className="mt-1 text-xs leading-5 text-ink/40">Based only on verified opportunities with documented values.</p></div></div></div>
