...
export function BenefitBrowser({ benefits, schoolName }: { benefits: Benefit[]; schoolName: string }) {
  ...
  const national = visible.filter((item) => item.scope === "national");
  const schoolSpecific = visible.filter((item) => item.scope === "school");

  const rows = (items: Benefit[]) => <div className="border-b-2 border-ink">
    {items.map((item, index)=><article key={item.slug} className="grid gap-4 border-b border-ink/20 bg-transparent py-5 last:border-0 md:grid-cols-[36px_1fr_160px_140px] md:items-center">
      <span className="hidden font-mono text-xs text-ink/30 md:block">{String(index+1).padStart(2,"0")}</span>
      <div><div className="mb-2 flex flex-wrap items-center gap-3"><span className="rule-label text-forest">{item.category}</span><span className="rule-label border-l border-ink/25 pl-3 text-ink/40">{item.scope === "national" ? "National benefit" : "School-specific benefit"}</span><StatusBadge status={item.status} /></div><h3 className="font-editorial text-xl font-bold"><Link href={`/benefits/${item.slug}`} className="hover:text-forest hover:underline">{item.name}</Link></h3><p className="mt-1 text-sm leading-5 text-ink/55">{item.description}</p><p className="mt-2 text-xs text-ink/40">Eligibility: {item.eligibility}</p></div>
  ...
  return <>
    ...
    <section className="mt-8" aria-labelledby="national-benefits"><p className="rule-label text-forest">Available broadly</p><h3 id="national-benefits" className="mt-2 font-editorial text-2xl font-bold">National student benefits</h3><p className="mb-4 mt-2 max-w-3xl text-sm leading-6 text-ink/50">These offers are available to many eligible students nationally. They are not presented as institutional benefits provided by {schoolName}.</p>{national.length ? rows(national) : <p className="border-y border-ink/20 py-6 text-sm text-ink/50">No national benefits match the current search and category filters.</p>}</section>

    <section className="mt-10" aria-labelledby="school-specific-benefits"><p className="rule-label text-forest">Official institution sources only</p><h3 id="school-specific-benefits" className="mt-2 font-editorial text-2xl font-bold">{schoolName}–specific benefits</h3>{schoolCount === 0 ? <div className="mt-4 border-y border-ink/20 bg-white py-6"><p className="font-bold">No verified school-specific benefits yet.</p><p className="mt-2 max-w-3xl text-sm leading-6 text-ink/50">UnlockED has not published any benefits as specific to {schoolName} without an official university source. The national offers above may still be available to eligible students.</p></div> : <><p className="mb-4 mt-2 max-w-3xl text-sm leading-6 text-ink/50">These benefits are linked only because an official {schoolName} source supports the listing.</p>{schoolSpecific.length ? rows(schoolSpecific) : <p className="border-y border-ink/20 py-6 text-sm text-ink/50">No school-specific benefits match the current search and category filters.</p>}</>}</section>
