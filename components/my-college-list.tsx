...
export function MyCollegeList({ initialItems }: { initialItems: Item[] }) {
  ...
  const decisionSeason=items.some(({record})=>Boolean(record.application?.decision));
  ...
  return <main className="min-h-screen px-5 pb-28 pt-12 sm:px-8 sm:pt-16"><div className="mx-auto max-w-6xl">
    <p className="rule-label text-forest">Your admissions workspace</p><div className="mt-3 flex flex-wrap items-end justify-between gap-6"><div><h1 className="font-editorial text-5xl font-semibold text-[var(--unlocked-text)]">My College List</h1><p className="mt-4 max-w-xl text-base leading-7 text-ink/52">The schools you are exploring and what you are doing next. No odds, labels, or pressure.</p></div><div className="flex flex-wrap gap-2"><Link href="/admissions/counselor" className="inline-flex min-h-11 items-center rounded-full border border-forest/20 px-4 text-sm font-bold text-forest hover:bg-mint/60">Application Counselor</Link><Link href="/cost-aid#colleges" className="inline-flex min-h-11 items-center rounded-full border border-forest/20 px-4 text-sm font-bold text-forest hover:bg-mint/60">Compare cost &amp; aid</Link><Link href="/admissions" className="inline-flex min-h-11 items-center rounded-full border border-forest/20 px-4 text-sm font-bold text-forest hover:bg-mint/60">Admissions Journey</Link><Link href="/colleges" className="inline-flex min-h-11 items-center rounded-full bg-ink px-4 text-sm font-bold text-white">Explore colleges</Link></div></div>
    ...
    {decisionSeason?<Link href="/admissions/decisions" className="mt-5 flex min-h-12 items-center justify-between rounded-2xl bg-forest px-5 text-sm font-bold text-white"><span>Decision Season is active</span><span>Review decisions and compare →</span></Link>:null}
    ...
    {visible.length ? <div className="mt-6 overflow-hidden rounded-[1.75rem] border border-ink/10 bg-[var(--unlocked-surface)] px-5 shadow-soft sm:px-8">{visible.map(({ college, record }) => {
      ...
      return <article key={college.id} className="grid gap-5 border-t border-ink/10 py-6 first:border-t-0 lg:grid-cols-[minmax(15rem,1.5fr)_minmax(11rem,.7fr)_minmax(12rem,.8fr)_auto] lg:items-center">
        ...
        <div className="flex flex-col items-stretch gap-1"><Link href={`/colleges/${college.slug}/application`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-ink/10 px-4 text-sm font-bold text-forest hover:bg-mint/60">Open workspace <ArrowIcon /></Link><Link href={`/colleges/${college.slug}#your-admissions-context`} className="px-3 py-1 text-center text-[11px] font-bold text-ink/42 hover:text-forest">Admissions context</Link></div>
      </article>;
