...
export function AdvisorPage() {
  ...
  if (loading) return <main className="min-h-[70vh] bg-white px-5 py-16 sm:px-8"><section className="mx-auto max-w-4xl"><p className="rule-label text-forest">Advisor</p><div className="mt-6 h-12 max-w-2xl rounded-2xl bg-paper" /><div className="mt-4 h-4 max-w-xl rounded-full bg-paper" /><div className="mt-10 h-44 rounded-[2rem] bg-paper" /></section></main>;
  if (!state || !recommendation) return <main className="min-h-[70vh] bg-white px-5 py-16 sm:px-8"><section className="mx-auto max-w-4xl"><p className="rule-label text-forest">Advisor</p><h1 className="mt-4 font-editorial text-5xl font-bold tracking-[-.045em]">Complete your profile first.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-ink/55">Advisor needs your saved school, major, year, goals, and activity before it can give useful guidance.</p><Link href="/profile" className="mt-8 inline-flex min-h-12 items-center justify-center rounded-full bg-forest px-6 text-sm font-bold text-white hover:bg-ink">Open profile</Link></section></main>;
    ...
    <section className="mx-auto max-w-4xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="mt-4 font-editorial text-5xl font-bold leading-[1] tracking-[-.05em] sm:text-7xl">What to do next.</h1>
          <p className="mt-5 max-w-2xl text-base leading-8 text-ink/55">A focused recommendation from your profile, saved activity, evidence, and current opportunity matches.</p>
        </div>
      ...
      <article className="mt-12 rounded-[2rem] bg-paper px-5 py-8 sm:px-8 sm:py-10">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          <Link href={opportunityHref(recommendation)} className="inline-flex min-h-12 items-center justify-center rounded-full bg-forest px-6 text-sm font-bold text-white transition hover:bg-ink">{relatedOpportunity ? "Open opportunity" : "Find matching opportunities"}</Link>
          <button type="button" onClick={completeRecommendation} className="inline-flex min-h-12 items-center justify-center rounded-full border border-forest/25 px-6 text-sm font-bold text-forest transition hover:border-forest hover:bg-white">Mark done</button>
        </div>
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-6">
          <InfoBlock title="Why now" values={recommendation.reasons.slice(0, 4)} />
        ...
        <aside className="h-fit rounded-[2rem] bg-white p-5 shadow-soft ring-1 ring-ink/8">
          <p className="rule-label text-forest">Advisor context</p>
...
function AdvisorAccessBadge({ access }: { access: AdvisorAccessState }) {
  const copy = access === "pro" ? "Pro advisor" : access === "preview" ? "Advisor preview" : access === "free" ? "Free" : "Needs profile";
