...
export function StageHome({ stage, firstName, collegeList }: { stage: Exclude<EducationalStage, "undergraduate">; firstName: string; collegeList?: { saved: number; active: number; decisions: number } }) {
  ...
  return <main className="min-h-[calc(100vh-5rem)] px-5 py-10 sm:px-8 sm:py-16">
    <div className="mx-auto max-w-6xl">
      <section className="relative overflow-hidden rounded-[2rem] border border-ink/10 bg-[var(--unlocked-surface)] px-6 py-12 shadow-soft sm:px-12 sm:py-16">
        ...
        {stage === "high_school" && collegeList ? <div className="mt-9 grid gap-px overflow-hidden rounded-2xl border border-ink/10 bg-ink/10 sm:grid-cols-[1.4fr_.6fr]">
          <Link href="/admissions" className="group bg-ink p-6 text-white"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-white/45">What matters right now</p><h2 className="mt-3 font-editorial text-2xl font-semibold">{collegeList.active ? "Continue your admissions plan" : collegeList.saved ? "Choose what to learn next" : "Find a college worth understanding"}</h2><p className="mt-2 text-sm text-white/55">Open your Admissions Journey →</p></Link>
          <Link href="/colleges/saved" className="bg-[var(--unlocked-surface)] p-6 hover:bg-mint/60"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-ink/38">My College List</p><p className="mt-3 font-editorial text-2xl font-semibold">{collegeList.saved} saved</p><p className="mt-2 text-xs text-ink/45">{collegeList.active} planned or applied · {collegeList.decisions} decisions</p></Link>
        </div> : null}
        <div className="mt-12 grid border-y border-ink/10 sm:grid-cols-2">
          {detail.priorities.map((priority, index) => <article key={priority.title} className={`py-6 sm:p-7 ${index % 2 === 0 ? "sm:border-r sm:border-ink/10" : ""} ${index > 1 ? "border-t border-ink/10" : index === 1 ? "border-t border-ink/10 sm:border-t-0" : ""}`}>
            ...
            {stage === "high_school" && priority.title === "Explore colleges" ? <Link href="/colleges" className="mt-4 inline-flex min-h-11 items-center text-sm font-bold text-forest">Open College Explorer →</Link> : stage === "high_school" && priority.title === "Plan your applications" ? <Link href="/admissions" className="mt-4 inline-flex min-h-11 items-center text-sm font-bold text-forest">Open Admissions Journey →</Link> : null}
          </article>)}
        </div>
