...
export function OpportunityCard({ opportunity, reasons }: { opportunity: Opportunity; reasons?: string[] }) {
  ...
  return <article className="flex h-full flex-col rounded-[1.5rem] bg-white/92 p-5 shadow-[0_16px_42px_rgba(43,33,26,.055)] ring-1 ring-ink/7 transition duration-200 hover:-translate-y-0.5 hover:bg-white hover:shadow-[0_22px_55px_rgba(43,33,26,.095)] motion-reduce:hover:translate-y-0">
    <div className="flex items-start justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><span className="rule-label text-forest">{opportunity.type}</span><StatusBadge status={opportunity.verification_status}/></div><SaveOpportunityButton opportunityId={opportunity.id} className="min-h-9 rounded-full border border-ink/12 px-3 text-ink/45 hover:border-forest hover:text-forest"/></div>
    ...
    <div className="mt-5 grid grid-cols-2 gap-3 text-xs"><div><p className="font-bold text-ink/35">{secondaryLabel}</p><p className="mt-1 font-black text-forest">{secondary}</p></div><div className="text-right"><p className="font-bold text-ink/35">Scope</p><p className="mt-1 font-black text-ink/65">{opportunity.school_scope}</p></div></div>
    <div className="mt-5 flex items-center gap-3"><Link href={`/opportunities/${opportunity.id}`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-forest px-4 text-sm font-bold text-white shadow-[0_12px_26px_rgba(31,95,67,.14)] hover:bg-ink">Open <ArrowIcon /></Link><a href={opportunity.official_source} target="_blank" rel="noreferrer" className="shrink-0 text-xs font-bold text-forest underline decoration-forest/30 underline-offset-4 hover:decoration-forest">Source</a></div>
  </article>;
}
