...
export function OpportunityCardContent({ opportunity, reasons, source }: { opportunity: OpportunityListing; reasons?: string[]; source?: "discover" | "for_you" }) {
  ...
  const showLifecycleBadge = ["closing_soon", "reopened", "upcoming", "closed"].includes(resolved.displayState);
  ...
  return <article data-ui-card="" data-discover-opportunity={source === "discover" ? opportunity.id : undefined} style={{ color: "var(--unlocked-text)", background: "var(--unlocked-surface)", borderColor: "var(--unlocked-border)" }} className="group flex h-full flex-col rounded-[1.1rem] border p-4 shadow-[0_12px_34px_rgba(43,33,26,.045)] transition duration-200 hover:border-forest/25 hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(43,33,26,.075)] focus-within:border-forest/35 focus-within:shadow-[0_18px_42px_rgba(43,33,26,.075)] motion-reduce:hover:translate-y-0 sm:p-5">
    <div className="flex min-h-6 flex-wrap items-center gap-2"><span className="rule-label text-forest">{cardType}</span>{showLifecycleBadge ? <LifecycleBadge state={resolved.displayState} confidence={resolved.confidence} label={resolved.label}/> : null}</div>
    <div className="mt-4 flex items-start gap-3"><OrganizationLogo opportunity={opportunity} size="md"/><div className="min-w-0 flex-1"><h3 className="font-editorial text-xl font-bold leading-[1.12] sm:text-2xl">{source === "discover" ? <DiscoverOpportunityLink href={detailHref} opportunityId={opportunity.id} category={opportunity.category} className={titleClass}>{opportunity.title}</DiscoverOpportunityLink> : <Link href={detailHref} className={titleClass}>{opportunity.title}</Link>}</h3><p className="mt-1.5 line-clamp-1 text-xs font-bold uppercase tracking-[.08em] text-ink/35">{opportunity.organization}</p></div></div>
    <div className="mt-4 min-w-0 flex-1"><p style={{ color: "var(--unlocked-muted)" }} className="line-clamp-2 text-sm leading-6">{opportunity.description}</p><p className="mt-2 line-clamp-1 text-xs font-semibold text-ink/40">{format}</p>{reasons?.length ? <details className="mt-4 rounded-xl bg-paper/70 px-4 py-3"><summary className="cursor-pointer text-xs font-bold text-ink/60">Why this matches</summary><ul className="mt-2 space-y-1 text-xs leading-5 text-ink/55">{reasons.map((reason)=><li key={reason}>{reason}</li>)}</ul></details> : null}</div>
    <dl className="mt-4 grid gap-x-4 gap-y-3 border-t border-ink/10 pt-4 text-xs sm:grid-cols-2">
      <CardFact label="Deadline" value={deadline} emphasis />
      <CardFact label="Value" value={value} />
      <div className="sm:col-span-2"><CardFact label="Eligibility" value={eligibilityLabel(opportunity)} /></div>
    </dl>
    <div className="mt-4 grid gap-2">{source === "discover" ? <DiscoverOpportunityLink href={detailHref} opportunityId={opportunity.id} category={opportunity.category} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-forest px-4 text-sm font-bold text-white shadow-[0_10px_24px_rgba(31,95,67,.13)] hover:bg-ink">Open Opportunity <ArrowIcon /></DiscoverOpportunityLink> : <Link href={detailHref} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-forest px-4 text-sm font-bold text-white shadow-[0_10px_24px_rgba(31,95,67,.13)] hover:bg-ink">Open Opportunity <ArrowIcon /></Link>}<AddToJourneyButton opportunityId={opportunity.id} className="rounded-xl border border-forest/30 bg-white px-4 text-forest hover:border-forest"/></div>
  </article>;
}
