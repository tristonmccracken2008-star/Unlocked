...
export function PersonalizedHome() {
  ...
  useEffect(() => {
    ...
    const onSession = (event: Event) => { const next = (event as CustomEvent<AccountSession>).detail; setSession(next); if (!next.authenticated) { clearLocalDashboardState();setProfile(null);setEditing(false);setJustCompleted(false);return; } const parsed = readCompletedStudentProfile(); if (parsed && schools.some((school) => school.slug === parsed.schoolSlug)) setProfile(parsed); else setProfile(null); };
  ...
  useEffect(() => {
    if (!ready) return;
    const view = profile && !editing && !justCompleted ? "dashboard" : !profile && !session?.authenticated ? "homepage" : "";
  ...
  }, [editing, justCompleted, profile, ready, session?.authenticated]);
    ...
    <section className="px-5 py-16 sm:px-8 sm:py-24">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-16 grid gap-8 border-y border-ink/15 py-8 md:grid-cols-3">
          <section>
            <p className="rule-label text-forest">What it is</p>
            <p className="mt-3 text-sm leading-7 text-ink/55">A maintained student opportunity directory, built to replace scattered bookmarks, newsletters, and missed deadlines.</p>
          </section>
          <section>
            <p className="rule-label text-forest">Why use it</p>
            <p className="mt-3 text-sm leading-7 text-ink/55">Useful programs often hide in university offices, provider pages, and eligibility fine print. UnlockED brings them into one account.</p>
          </section>
          <section>
            <p className="rule-label text-forest">How it earns trust</p>
            <p className="mt-3 text-sm leading-7 text-ink/55">Listings prioritize official sources, verification status, last reviewed dates, and clear application or claim steps.</p>
          </section>
        </div>
      </div>
    </section>
    ...
    <section className="border-b-2 border-ink bg-paper px-5 py-10 sm:px-8"><div className="mx-auto max-w-2xl border-x border-ink/20">
      <div className="bg-white px-5 py-7 sm:px-8"><div className="flex items-center justify-between"><div><p className="rule-label text-forest">Find My Opportunities</p><p className="mt-1 text-xs text-ink/40">Step {step} of 5 · Saved to your account</p></div>{onCancel&&<button type="button" onClick={onCancel} className="min-h-11 px-2 text-xs font-bold uppercase tracking-wider text-ink/50">Cancel</button>}</div><div className="mt-3 grid grid-cols-5 gap-2" aria-hidden="true">{[1,2,3,4,5].map((item)=><span key={item} className={`h-1 ${item<=step?"bg-gold":"bg-ink/10"}`}/>)}</div><form onSubmit={submit} className="mt-5 space-y-4">
        ...
        <div className="relative pt-1"><label htmlFor="profile-school" className="mb-2 block text-xs font-bold uppercase tracking-wider text-ink/60">School or university</label><div className={`flex h-13 items-center gap-3 border bg-paper/40 px-4 transition-colors focus-within:border-forest focus-within:bg-white ${selectedSchool?"border-forest":"border-ink/25"}`}><SearchIcon className="h-4 w-4 shrink-0 text-ink/35"/><input id="profile-school" value={schoolQuery} onFocus={()=>setShowSuggestions(true)} onChange={(event)=>{setSchoolQuery(event.target.value);setSelectedSchool(null);setShowSuggestions(true)}} placeholder="Try UChicago, University of Michigan, or umich.edu" autoComplete="off" aria-controls="profile-school-suggestions" aria-expanded={showSuggestions&&Boolean(normalized)} className="min-w-0 flex-1 bg-transparent text-[15px] outline-none placeholder:text-ink/30"/></div>
          {showSuggestions&&normalized&&matches.length>0&&<div id="profile-school-suggestions" role="listbox" aria-label="Matching schools" className="absolute z-20 mt-1 w-full border-2 border-ink bg-white shadow-[6px_6px_0_#2b211a]"><p className="px-3 py-2 rule-label text-ink/40">Top matches</p>{matches.map((school)=><button key={school.slug} type="button" role="option" aria-selected={school.slug===selectedSchool?.slug} onMouseDown={(event)=>event.preventDefault()} onClick={()=>choose(school)} className="block w-full border-t border-ink/15 px-3 py-2.5 text-left hover:bg-paper"><span className="block text-sm font-bold">{school.name}</span><span className="block text-xs text-ink/45">{school.domain} · {school.location}</span></button>)}</div>}
          {showSuggestions&&normalized&&matches.length===0&&<div id="profile-school-suggestions" className="absolute z-20 mt-1 w-full border-2 border-ink bg-white p-4 shadow-[6px_6px_0_#2b211a]"><p className="font-bold">School not found</p><Link href={`/contact?school=${encodeURIComponent(schoolQuery)}`} className="mt-2 inline-block border-b border-forest text-sm font-bold text-forest">Request this school</Link></div>}
...
function OnboardingComplete({ profile, onContinue }: { profile: StudentProfile; onContinue: () => void }) {
  ...
  return <main className="mx-auto min-h-[72vh] max-w-5xl px-5 py-10 sm:px-8 sm:py-14"><section className="border-y border-ink/20 bg-white px-5 py-9 sm:px-10 sm:py-12"><div className="mx-auto max-w-3xl text-center"><p className="rule-label text-forest">Profile complete</p><h1 className="mt-3 font-editorial text-4xl font-bold leading-tight sm:text-5xl">Your dashboard is ready.</h1><p className="mx-auto mt-4 max-w-xl text-base leading-7 text-ink/55">We found <strong className="text-ink">{matches.length} opportunities</strong> based on your school, major, year, and goals.</p></div>
    ...
    <div className="mt-8 text-center"><button type="button" onClick={onContinue} className="inline-flex min-h-12 items-center gap-2 bg-ink px-7 text-sm font-bold uppercase tracking-wider text-white hover:bg-forest">Open my dashboard <ArrowIcon/></button><p className="mt-4 text-xs text-ink/40">Your profile and progress sync to your account.</p></div>
  </section></main>;
}
