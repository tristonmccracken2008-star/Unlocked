...
function StudentDashboard({ profile, onEdit }: { profile: StudentProfile; onEdit: () => void }) {
  ...
  return <div className="mx-auto max-w-6xl bg-white px-5 py-14 sm:px-8 sm:py-20">
    <section className="pb-20 sm:pb-24">{focus&&<div className="grid gap-10 bg-paper px-6 py-10 shadow-[0_18px_50px_rgba(16,36,62,.06)] sm:px-10 sm:py-12 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-end"><div><div className="flex flex-wrap items-center justify-between gap-4"><p className="rule-label text-forest">Today’s Focus</p><button onClick={onEdit} className="text-xs font-bold uppercase tracking-wider text-ink/40 hover:text-forest">Edit profile</button></div><p className="mt-8 text-xs font-bold uppercase tracking-wider text-ink/30">{focus.opportunity.organization}</p><h1 className="mt-3 max-w-4xl font-editorial text-4xl font-bold leading-[1.05] tracking-[-.04em] sm:text-6xl">{focus.opportunity.title}</h1><p className="mt-6 max-w-2xl text-base leading-8 text-ink/50">{focus.reasons[0]?`Selected because it is a ${focus.reasons[0].toLowerCase()}.`:"Selected because it closely matches your school, major, year, and interests."}</p><p className="mt-5 text-xs text-ink/35">{verifiedMatches.length} verified matches · {school.name} · {profile.major} · {profile.year}</p></div><div><p className="rule-label text-ink/30">Estimated value</p><p className="mt-2 font-editorial text-3xl font-bold text-forest">{valueLabel(focus.opportunity.estimated_value)}</p><Link href={`/opportunities/${focus.opportunity.id}`} className="mt-7 flex min-h-12 items-center justify-center bg-ink px-5 text-xs font-bold uppercase tracking-wider text-white hover:bg-forest">Explore Opportunity</Link></div></div>}
    </section>
