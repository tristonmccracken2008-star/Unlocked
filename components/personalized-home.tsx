...
function StudentDashboard({ profile, onEdit }: { profile: StudentProfile; onEdit: () => void }) {
  ...
  return <div className="mx-auto max-w-6xl bg-white px-5 py-14 sm:px-8 sm:py-20">
    <section className="pb-20 sm:pb-24"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start"><div><p className="rule-label text-forest">Your UnlockED</p><h1 className="mt-4 max-w-4xl font-editorial text-5xl font-bold leading-[1.02] tracking-[-.045em] sm:text-7xl">Welcome back, Triston.</h1><p className="mt-7 max-w-2xl text-xl leading-9 text-ink/50">We found <strong className="font-semibold text-ink">{verifiedMatches.length} verified opportunities</strong> that match your profile.</p></div><button onClick={onEdit} className="self-start text-xs font-bold uppercase tracking-wider text-ink/45 hover:text-forest">Edit profile</button></div><div className="mt-8 flex flex-wrap gap-2"><span className="bg-paper px-3 py-2 text-xs font-bold text-ink/65">{school.name}</span><span className="bg-paper px-3 py-2 text-xs font-bold text-ink/65">{profile.major}</span><span className="bg-paper px-3 py-2 text-xs font-bold text-ink/65">{profile.year}</span><span className="max-w-full truncate bg-paper px-3 py-2 text-xs font-bold text-ink/65">{profile.topics?.join(" · ") || profile.interests}</span></div>
      {focus&&<div className="mt-16 grid gap-8 bg-paper px-6 py-9 shadow-[0_16px_45px_rgba(16,36,62,.05)] sm:px-10 sm:py-10 lg:grid-cols-[minmax(0,1fr)_210px] lg:items-end"><div><p className="rule-label text-forest">Today’s Focus</p><p className="mt-5 text-xs font-bold uppercase tracking-wider text-ink/30">{focus.opportunity.organization}</p><h2 className="mt-2 max-w-3xl font-editorial text-3xl font-bold leading-tight tracking-[-.02em] sm:text-5xl">{focus.opportunity.title}</h2><p className="mt-5 max-w-2xl text-sm leading-7 text-ink/50">{focus.reasons[0]?`Selected because it is a ${focus.reasons[0].toLowerCase()}.`:"Selected because it closely matches your school, major, year, and interests."}</p></div><div><p className="rule-label text-ink/30">Estimated value</p><p className="mt-2 font-editorial text-2xl font-bold text-forest">{valueLabel(focus.opportunity.estimated_value)}</p><Link href={`/opportunities/${focus.opportunity.id}`} className="mt-6 flex min-h-12 items-center justify-center bg-ink px-5 text-xs font-bold uppercase tracking-wider text-white hover:bg-forest">Explore Opportunity</Link></div></div>}
    </section>
