...
export function StudentSetup({ onSave, initialProfile, onCancel }: { onSave: (profile: StudentProfile) => void; initialProfile: StudentProfile | null; onCancel?: () => void }) {
  ...
  return <>
    <section className="border-b-2 border-ink bg-paper"><div className="mx-auto grid max-w-6xl border-x border-ink/20 lg:grid-cols-[1.05fr_.95fr]">
      ...
      <div className="bg-white px-5 py-7 sm:px-8"><div className="flex items-center justify-between"><div><p className="rule-label text-forest">{initialProfile ? "Edit your profile" : "Build your profile"}</p><p className="mt-1 text-xs text-ink/40">Step {step} of 3 · Stored only in this browser</p></div>{onCancel&&<button type="button" onClick={onCancel} className="min-h-11 px-2 text-xs font-bold uppercase tracking-wider text-ink/50">Cancel</button>}</div><div className="mt-3 grid grid-cols-3 gap-2" aria-hidden="true">{[1,2,3].map((item)=><span key={item} className={`h-1 ${item<=step?"bg-gold":"bg-ink/10"}`}/>)}</div><form onSubmit={submit} className="mt-5 space-y-4">
        {step===1&&<>
        ...
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Year</span><select value={year} onChange={(event)=>setYear(event.target.value)} required className="h-11 w-full border-2 border-ink bg-white px-3 outline-none"><option value="">Select your year</option>{years.map((item)=><option key={item}>{item}</option>)}</select></label>
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Major</span><select value={major} onChange={(event)=>setMajor(event.target.value)} required className="h-11 w-full border-2 border-ink bg-white px-3 outline-none"><option value="">Select your major</option>{major&&!opportunityMajors.includes(major as (typeof opportunityMajors)[number])&&<option value={major}>{major}</option>}{opportunityMajors.filter((item)=>item!=="All"&&item!=="Any Major").map((item)=><option key={item}>{item}</option>)}</select></label>
        </>}
        {step===2&&<>
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Minor <span className="font-normal text-ink/40">(optional)</span></span><input value={minor} onChange={(event)=>setMinor(event.target.value)} placeholder="e.g. Mathematics" className="h-11 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Clubs <span className="font-normal text-ink/40">(optional)</span></span><input value={clubs} onChange={(event)=>setClubs(event.target.value)} placeholder="e.g. Robotics Club, Women in Finance" className="h-11 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
        </>}
        {step===3&&<>
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Career goal</span><input value={careerGoal} onChange={(event)=>setCareerGoal(event.target.value)} required placeholder="e.g. Quantitative researcher" className="h-11 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
        <label className="block"><span className="mb-1.5 block text-sm font-bold">Interests</span><input value={interests} onChange={(event)=>setInterests(event.target.value)} required placeholder="e.g. Quant, AI, research" className="h-11 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
        </>}
        <div className="flex gap-3">{step>1&&<button type="button" onClick={()=>setStep(step-1)} className="min-h-12 border-2 border-ink px-5 text-sm font-bold uppercase tracking-wider">Back</button>}<button type="submit" className="flex min-h-12 flex-1 items-center justify-center gap-2 bg-ink px-5 text-sm font-bold uppercase tracking-wider text-white hover:bg-forest">{step===3?(initialProfile?"Save profile":"Open my dashboard"):"Continue"} <ArrowIcon /></button></div>
