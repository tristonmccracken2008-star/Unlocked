...
type StudentProfile = { schoolSlug: string; year: string; major: string; interests?: string; careerGoals?: string };
...
function StudentSetup({ onSave }: { onSave: (profile: StudentProfile) => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    onSave({ schoolSlug: school.slug, year, major: major.trim(), interests: interests.trim(), careerGoals: careerGoals.trim() });
  }
  ...
  return <>
    <section className="border-b-2 border-ink bg-paper"><div className="mx-auto grid max-w-7xl border-x border-ink/20 lg:grid-cols-[.85fr_1.15fr]">
      ...
      <div className="bg-white px-5 py-12 sm:px-8 lg:p-12"><p className="rule-label text-ink/40">Student setup</p><form onSubmit={submit} className="mt-7 space-y-7">
        ...
        <label className="block"><span className="mb-2 block text-sm font-bold">Interests <span className="font-normal text-ink/40">(optional)</span></span><input value={interests} onChange={(event)=>setInterests(event.target.value)} placeholder="e.g. AI, research, startups" className="h-12 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
        <label className="block"><span className="mb-2 block text-sm font-bold">Career goals <span className="font-normal text-ink/40">(optional)</span></span><input value={careerGoals} onChange={(event)=>setCareerGoals(event.target.value)} placeholder="e.g. Software engineering, quantitative finance" className="h-12 w-full border-2 border-ink px-3 outline-none placeholder:text-ink/35"/></label>
...
function StudentDashboard({ profile, onEdit }: { profile: StudentProfile; onEdit: () => void }) {
  ...
  return <div className="mx-auto max-w-7xl border-x border-ink/20 bg-paper">
    ...
    <RecommendationSection school={school} major={profile.major} year={profile.year} interests={profile.interests} careerGoals={profile.careerGoals} />
