...
function StudentDashboard({ profile, onEdit }: { profile: StudentProfile; onEdit: () => void }) {
  ...
  return <div className="mx-auto max-w-6xl bg-white px-5 py-12 sm:px-8 sm:py-16">
    <section className="pb-14"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><p className="rule-label text-forest">Your UnlockED</p><h1 className="mt-3 font-editorial text-4xl font-bold tracking-[-.035em] sm:text-6xl">Welcome back, Triston.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-ink/55">We found <strong className="text-ink">{verifiedMatches.length} verified opportunities</strong> that match your profile.</p></div><button onClick={onEdit} className="self-start border-b border-ink/25 pb-1 text-xs font-bold uppercase tracking-wider hover:border-forest hover:text-forest">Edit profile</button></div><div className="mt-7 flex flex-wrap gap-2"><span className="bg-paper px-3 py-2 text-xs font-bold">{school.name}</span><span className="bg-paper px-3 py-2 text-xs font-bold">{profile.major}</span><span className="bg-paper px-3 py-2 text-xs font-bold">{profile.year}</span><span className="max-w-full truncate bg-paper px-3 py-2 text-xs font-bold">{profile.topics?.join(" · ") || profile.interests}</span></div>
      {focus&&<div className="mt-12 grid gap-7 bg-paper px-5 py-7 sm:px-8 lg:grid-cols-[minmax(0,1fr)_210px] lg:items-end"><div><p className="rule-label text-forest">Today’s Focus</p><p className="mt-4 text-xs font-bold uppercase tracking-wider text-ink/35">{focus.opportunity.organization}</p><h2 className="mt-2 font-editorial text-3xl font-bold leading-tight sm:text-4xl">{focus.opportunity.title}</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-ink/55">{focus.reasons[0]?`Selected because it is a ${focus.reasons[0].toLowerCase()}.`:"Selected because it closely matches your school, major, year, and interests."}</p></div><div><p className="rule-label text-ink/35">Estimated value</p><p className="mt-2 font-editorial text-2xl font-bold text-forest">{valueLabel(focus.opportunity.estimated_value)}</p><Link href={`/opportunities/${focus.opportunity.id}`} className="mt-5 flex min-h-12 items-center justify-center bg-ink px-5 text-xs font-bold uppercase tracking-wider text-white hover:bg-forest">Explore Opportunity</Link></div></div>}
    </section>
