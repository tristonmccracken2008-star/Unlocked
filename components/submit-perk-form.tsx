...
export function SubmitPerkForm() {
  ...
  return <form onSubmit={submit} className="mt-10 space-y-6 border-y-2 border-ink bg-white p-6 sm:p-8">
    {saved && <div role="status" className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">Thanks—your submission was saved in this browser for review.</div>}
    <div className="grid gap-5 sm:grid-cols-2"><Field label="Perk name" name="name" placeholder="e.g. Figma for Education" required /><label className="block"><span className="mb-2 block text-sm font-bold">Category</span><select name="category" className="h-12 w-full rounded-xl border border-ink/15 bg-white px-3 outline-none focus:border-forest focus:ring-2 focus:ring-lime">{categories.slice(1).map((category) => <option key={category}>{category}</option>)}</select></label></div>
    ...
    <label className="block"><span className="mb-2 block text-sm font-bold">Description</span><textarea name="description" rows={5} required placeholder="What does the perk include, and who is eligible?" className="w-full resize-none rounded-xl border border-ink/15 px-3 py-3 outline-none placeholder:text-ink/30 focus:border-forest focus:ring-2 focus:ring-lime" /></label>
    ...
    <button type="submit" className="w-full rounded-xl bg-forest px-6 py-3.5 font-bold text-white hover:bg-ink sm:w-auto">Save submission</button>
  </form>;
}
...
function Field({ label, name, type = "text", placeholder, required = false }: { label: string; name: string; type?: string; placeholder?: string; required?: boolean }) {
  return <label className="block"><span className="mb-2 block text-sm font-bold">{label}</span><input name={name} type={type} placeholder={placeholder} required={required} className="h-12 w-full rounded-xl border border-ink/15 px-3 outline-none placeholder:text-ink/30 focus:border-forest focus:ring-2 focus:ring-lime" /></label>;
}
