...
export function Header() {
  return <header className="border-b-2 border-ink bg-paper">
    ...
    <nav aria-label="Mobile navigation" className="scrollbar-none mx-auto flex max-w-7xl overflow-x-auto border-t border-ink/15 px-5 text-[11px] font-bold uppercase tracking-wider sm:hidden">{[["Dashboard","/"],["Opportunities","/opportunities"],["University","/university"],["Career","/career"],["Profile","/profile"]].map(([label,href])=><Link key={href} href={href} className="flex min-h-11 shrink-0 items-center border-r border-ink/15 px-4 first:border-l hover:bg-white hover:text-forest">{label}</Link>)}</nav>
  </header>;
}
