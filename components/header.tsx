...
export function Header() {
  ...
  const destinations=[["Discover","/opportunities"],["For You","/advisor"],["Journey","/"],["Refer","/referral"]];
  ...
  return <>
    ...
    <nav aria-label="Mobile navigation" className="fixed inset-x-4 bottom-4 z-40 grid grid-cols-4 rounded-full bg-ink/95 p-1 text-xs font-bold text-white shadow-[0_20px_60px_rgba(43,33,26,.24)] backdrop-blur sm:hidden">{destinations.map(([label,href])=>{const active=href==="/"?pathname==="/":pathname?.startsWith(href);return <Link key={href} href={href} aria-current={active?"page":undefined} className={`rounded-full px-3 py-3 text-center transition duration-200 ${active?"bg-white text-forest":"text-white/70 hover:text-white"}`}>{label}</Link>})}</nav>
  </>;
}
