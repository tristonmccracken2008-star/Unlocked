...
export function Header() {
  ...
  return <>
    <header className="border-b border-ink/10 bg-paper/95 backdrop-blur"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-x-6 gap-y-2 px-5 py-3 sm:px-8"><Logo/><nav aria-label="Primary navigation" className="order-3 hidden w-full gap-1 overflow-x-auto text-sm font-bold text-ink/55 sm:order-none sm:flex sm:w-auto">{destinations.map(([label,href])=>{const active=href==="/"?pathname==="/":pathname?.startsWith(href);return <Link key={href} href={href} aria-current={active?"page":undefined} className={`rounded-full px-4 py-2 transition ${active?"bg-white text-forest shadow-soft":"hover:bg-white hover:text-forest"}`}>{label}</Link>})}</nav><div className="flex items-center gap-3"><Link href="/profile" className={`rounded-full px-3 py-2 text-xs font-bold ${pathname?.startsWith("/profile")?"bg-white text-forest":"text-ink/45 hover:bg-white hover:text-forest"}`}>Profile</Link><AccountButton compact/></div></div></header>
    <nav aria-label="Mobile navigation" className="fixed inset-x-4 bottom-4 z-40 grid grid-cols-3 rounded-full bg-ink p-1 text-xs font-bold text-white shadow-soft sm:hidden">{destinations.map(([label,href])=>{const active=href==="/"?pathname==="/":pathname?.startsWith(href);return <Link key={href} href={href} aria-current={active?"page":undefined} className={`rounded-full px-3 py-3 text-center transition ${active?"bg-white text-forest":"text-white/70"}`}>{label}</Link>})}</nav>
  </>;
}
