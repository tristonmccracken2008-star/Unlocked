...
const commandActions = read("components/journey-command-actions.tsx");
const styles = read("components/journey-timeline.module.css");
const page = read("app/page.tsx");
const loading = read("app/loading.tsx");
const journeyRecoveryLoading = read("components/student-journey-dashboard.tsx");
...
for (const required of ["See what you are pursuing and what needs attention next.", "Needs attention", "Active opportunities", "Professional history", "Journey Cards"]) assert.ok(commandCenter.includes(required), `Command center must render ${required}.`);
...
assert.ok(journeyRecoveryLoading.includes("Opening your Journey") && journeyRecoveryLoading.includes("Opening your story."), "Authenticated Journey recovery must retain calm, Journey-specific copy.");
