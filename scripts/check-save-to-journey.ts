...
assert.match(activityStyles, /\.saveButton:disabled[\s\S]*cursor:\s*default/, "Saving must not invoke a native busy cursor that can linger after success.");
...
assert.doesNotMatch(motion, /navigator\.vibrate|data-journey-save-burst|confirmationQueue/, "Routine Journey saves must not vibrate, emit particles, or duplicate the inline confirmation.");
assert.doesNotMatch(motionStyles, /journey-save-ring|journey-save-particle|journey-save-chip/);
