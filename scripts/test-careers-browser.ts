...
try {
  ...
  await page.goto(`${origin}/careers`, { waitUntil: "domcontentloaded", timeout: 60000 }); await page.getByRole("heading", { name: "Explore Careers" }).waitFor(); await page.getByText(/181 career paths/).waitFor(); await noOverflow(page, "Careers 1440");
  const search = page.getByRole("searchbox", { name: "Search careers" }); await page.waitForTimeout(1000); await page.keyboard.press("/"); assert.equal(await search.evaluate((element) => element === document.activeElement), true, "The slash shortcut must focus career search."); await search.fill("quant"); await page.locator('[data-career-card="quantitative-trader"]').waitFor();
  await search.fill(""); const compare = page.getByRole("button", { name: "+ Compare" }); await compare.nth(0).click(); await compare.nth(1).click(); await page.getByRole("heading", { name: "Compare tradeoffs, not winners." }).waitFor(); await page.getByLabel("Major differences").waitFor();
  await page.getByRole("link", { name: "Accountant", exact: true }).first().click(); await page.getByRole("heading", { name: "Accountant", exact: true }).waitFor(); await page.getByRole("heading", { name: "Eight lenses, with reasons" }).waitFor(); await page.getByRole("heading", { name: "Safe opportunities" }).waitFor(); await page.getByRole("heading", { name: "Try this career before committing" }).waitFor(); assert.match((await page.getByRole("link", { name: "Find related opportunities" }).getAttribute("href")) ?? "", /^\/opportunities\?query=/); assert.ok(await page.locator('[data-career-detail="accountant"] a[href^="/careers/"]').count(), "Career details must link to related careers."); await noOverflow(page, "Career detail 1440"); await page.screenshot({ path: `${output}/accountant-1440.png`, fullPage: true }); await page.goto(`${origin}/careers/quantitative-trader`, { waitUntil: "domcontentloaded" }); await page.getByText("45–60 hours", { exact: true }).waitFor(); await page.getByText("BLS occupation median—not total compensation", { exact: true }).waitFor(); await page.getByText(/backtest can fail once fees/).waitFor(); noErrors(); await desktop.close();
