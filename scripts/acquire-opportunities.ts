...
const acceptedIds = new Set(allOpportunityAcquisitionRecords.map((record) => record.id));
...
for (const candidate of sortAcquisitionQueue(allOpportunityAcquisitionCandidates)) {
...
if (diagnostics.missingEvidence.length || diagnostics.gateFailures.length || diagnostics.duplicates.length) {
  console.error(JSON.stringify({ batchIds: opportunityAcquisitionBatches.map((batch) => batch.batchId), mode: write ? "write" : "dry-run", diagnostics }, null, 2));
  process.exitCode = 1;
} else {
