...
const journeyEditorial = read("components/journey-command-center.tsx");
const journeyEditorialModel = read("lib/journey-timeline.ts");
...
for (const label of [
  "Journey",
  "See what you are pursuing and what needs attention next.",
  "JourneyTimelineControl",
  "JourneyCardEntry",
]) {
  assert.ok(journeyEditorial.includes(label), `Journey command-center experience must render ${label}.`);
}
assert.ok(journeyEditorial.includes("data-journey-command-center"), "Journey must expose one primary command-center region.");
...
assert.ok(journeyPage.includes("buildJourneyCommandCenterModel"), "Journey route must compose the command center on the server.");
assert.ok(journeyEditorialModel.includes("record.history"), "Journey timeline must derive events from canonical saved progress.");
assert.doesNotMatch(journeyEditorialModel, /createAdvisorProfile|getRoadmap|buildPathprint/, "Journey timeline must not duplicate Advisor guidance or geometry work.");
assert.ok(!journeyEditorial.includes("Journey progress"), "Journey must not restore the retired progress dashboard framing.");
assert.ok(journeyEditorial.includes("Active opportunities"), "Journey must make current application management easy to scan.");
assert.doesNotMatch(journeyEditorial, /Your next step|Horizon|recommendation/i, "Journey must not compete with the Advisor experience.");
