...
for (const section of ["Your private record of what you saved, pursued, and accomplished.", "Things to do", "Active opportunities", "History", "Journey Cards"]) assert.ok(commandCenter.includes(section), `Journey command center must include ${section}.`);
assert.ok(commandModel.includes("slice(0, 3)") && commandModel.includes("historyLimit"), "Things to do and initial History must remain bounded.");
