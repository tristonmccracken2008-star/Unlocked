...
for (const section of ["Opportunity command center", "Needs attention", "Active opportunities", "History", "Journey Cards"]) assert.ok(commandCenter.includes(section), `Journey command center must include ${section}.`);
assert.ok(commandModel.includes("slice(0, 5)") && commandModel.includes("historyLimit"), "Attention and initial History must remain bounded.");
