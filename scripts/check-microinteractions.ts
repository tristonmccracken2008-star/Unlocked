...
assert.match(saveMotion, /transferQueue/);
assert.match(saveMotion, /cancelJourneySaveMotion/);
assert.match(saveMotion, /prefers-reduced-motion: reduce/);
assert.match(saveMotion, /if \(prefersReducedMotion\(\)\) return/);
assert.match(saveMotion, /duration: 360/);
assert.doesNotMatch(saveMotion, /navigator\.vibrate/, "Web interactions must not assume haptic hardware.");
assert.doesNotMatch(saveMotion, /showSuccessBurst|confirmationQueue|data-journey-save-chip/, "Routine Journey saves must use the inline authoritative state instead of particles or duplicate toasts.");
...
assert.doesNotMatch(saveButtonStyles, /save-progress-trace/);
...
assert.match(globalStyles, /--motion-deliberate:\s*320ms/);
...
assert.doesNotMatch(globalStyles, /\[data-filter-results\] > \*/, "Large Discover result sets must not animate every child.");
